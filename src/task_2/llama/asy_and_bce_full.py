...
if __name__ == '__main__':
    ...
    class MultiLabelTrainerASYM(Trainer):
    ...
    class MultiLabelTrainerBCE(Trainer):
        def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
            labels = inputs.pop("labels").float()
            outputs = model(**inputs)
            logits = outputs.logits

            loss_fct = torch.nn.BCEWithLogitsLoss(reduction='none') #pos_weight=class_weights, 
            loss = loss_fct(logits, labels)
            loss = loss.mean()
    ...
    results_csv_path = os.path.join(output_dir, f"paragraph-multilabel-llama-{model_type}-{loss_type}-seed-{seed}.csv")
