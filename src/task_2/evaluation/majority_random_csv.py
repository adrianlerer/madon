...
input_csv = 'llama-cpt-bce/peft_multi_label_cpt_seed_32.csv'
...
with open(input_csv, newline='') as f:
    ...
    for row in reader:
        ...
        if len(gold) != 8 or not set(gold).issubset({'0', '1'}):
            raise ValueError(f"Invalid label: {gold}")

        rand_pred = ''.join(random.choice('01') for _ in range(8))

        random_preds.append({'Gold Labels': gold, 'Predicted Labels': rand_pred})
        zero_preds.append({'Gold Labels': gold, 'Predicted Labels': '00000000'})
